...
// Legacy 5-dimension config for backward compatibility
const dimensions = [
...
function getScore(item: EvaluationSummary, key: string): number {
  // Try new dimensionScores format first
  if (item.dimensionScores?.length) {
    const dim = item.dimensionScores.find(d => d.key === key)
    if (dim) return dim.score
  }
  // Legacy fallback
  ...
  return (legacy[scoreMap[key]] as number) ?? 0
}
